...
public class Verified : PageModel
{
    ...
    public async Task<IActionResult> OnPostSeedSelfishness()
    {
        ...
        {
            await _publishEndpoint.Publish(new SeedSelfishness(verifiedPlEntry.EntryId));
        }
    ...
    public async Task<IActionResult> OnPostUpdateEntry(UpdateEntry model, UpdateAction action)
    {
        ...
        else
        {
            ...
            await _repo.Insert(new VerifiedEntry(
                model.EntryId, model.FullName, model.EntryTeamName,
                model.VerifiedEntryType, model.Alias, model.Description));
    ...
    public async Task<IActionResult> OnPostAddEntry(AddEntry model)
    {
        ...
        await _repo.Insert(new VerifiedEntry(
            model.EntryId, model.FullName, model.EntryTeamName,
            model.VerifiedEntryType.Value, model.Alias, model.Description));
...
public enum UpdateAction { Save, Del }
