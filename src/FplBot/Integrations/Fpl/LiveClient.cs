...
public class LiveClient(HttpClient httpClient, ICacheProvider client) : ILiveClient
{
    ...
    {
        var response = await client.GetCachedOrFetch<LiveResponse>($"/api/event/{gameweek}/live/", httpClient.GetStringAsync,
            isOngoingGameweek ? TimeSpan.FromMinutes(5) : TimeSpan.FromHours(24));
