...
public class WebPushDispatchTests(AppFixture fixture) : IAsyncLifetime
{
    ...
    [Fact]
    public async Task LikelyPriceChanges_SubscriberOnlySubscribedToPriceChanges_GetsNothing()
    {
        var endpoint = $"https://push.example.test/{Guid.NewGuid():N}";
        var subscriberId = await fixture.SubscribeToWebPush(leagueId: 123, endpoint: endpoint);
    ...
    [Fact]
    public async Task LikelyPriceChanges_SubscriberOnlySubscribedToAll_GetsNotified()
    {
        var endpoint = $"https://push.example.test/{Guid.NewGuid():N}";
        var subscriberId = await fixture.SubscribeToWebPush(leagueId: 123, endpoint: endpoint);
        var response = await fixture.PutWebPush(subscriberId, "/api/web/me/events", new { events = new[] { "All" } });
        response.EnsureSuccessStatusCode();
