...
namespace FplBot.Tests.UnitTests.StringParsers;
...
public class MessageHelperTests
{
    ...
    {
        var resultOne = MessageHelper.ExtractArgs("one standings, transfers, captains", ["one {args}", "two {args}"]);
        Assert.Equal("standings, transfers, captains", resultOne);

        var resultTwo = MessageHelper.ExtractArgs("two standings, transfers, captains", ["one {args}", "two {args}"]);
